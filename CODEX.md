...
Sometimes songs don't show up in Spotify Wrapped or Airbuds — even when you listened to them. Users have no visibility into *why*. We're building a tracker that makes that process transparent. 
...
Spotify publicly states a stream counts after **30 seconds** of playback.

| Progress Reached | Our Label |
|---|---|
| < 30 seconds | ❌ Likely did NOT count |
| ≥ 30 seconds | ✅ Likely counted |
| ≥ 90% of track | 🔥 Very likely counted + appears in history |

We then compare our prediction against whether the song later shows up in `/me/player/recently-played` to compute real accuracy over time.
...
> "If a song reaches 30 seconds of playback, how often does it appear in Spotify recently played / Airbuds history?"
...
- Does our prediction match? (accuracy)
...
4. **Stats Dashboard** — Accuracy score, skip rate, finish rate, fun insights
...
- Write polling logic for `/currently-playing` (every 3–5 seconds)
...
- Spotify's public API **does not expose an official "this stream counted" flag** — our predictions are estimates based on the published 30-second rule
- `recently-played` is listening history, not an internal stream-accounting ledger
- Airbuds is used as a **manual visual check** in the demo, not programmatic integration
...
├── player.html         # Now Playing tracker
├── history.html        # Session history
├── dashboard.html      # Stats + accuracy
├── style.css
├── app.js              # Core logic + Spotify polling
